...
interface IMigratorChef {
    // Perform LP token migration from legacy PanFruitSwap to FruitSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to PanFruitSwap LP tokens.
    // FruitSwap must mint EXACTLY the same amount of FruitSwap LP tokens or
    // else something bad will happen. Traditional PanFruitSwap does not
